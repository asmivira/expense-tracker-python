...
def view_expense_details():
	...
	date.set_date(expenditure_date) ; pay.set(values[2]) ; desc.set(values[3]) ; amnt.set(values[4]) ; MoP.set(values[5])


def clear_fields():
	global desc, pay, amnt, MoP, date, table

	today_date = datetime.datetime.now().date()

	desc.set('') ; pay.set('') ; amnt.set(0.0) ; MoP.set('Cash'), date.set_date(today_date)
...
def add_another_expense():
	...
	if not date.get() or not pay.get() or not desc.get() or not amnt.get() or not MoP.get():
		...
		(date.get_date(), pay.get(), desc.get(), amnt.get(), MoP.get())
		)
		connector.commit()
...
def edit_expense():
	...
	def edit_existing_expense():
		...
		connector.execute('UPDATE ExpenseTracker SET Date = ?, Payee = ?, Description = ?, Amount = ?, ModeOfPayment = ? WHERE ID = ?',
		                  (date.get_date(), pay.get(), desc.get(), amnt.get(), MoP.get(), contents[0]))
		connector.commit()
...
def expense_to_words_before_adding():
	global date, desc, amnt, pay, MoP

	if not date or not desc or not amnt or not pay or not MoP:
		mb.showerror('Incomplete data', 'The data is incomplete, meaning fill all the fields first!')

	message = f'Your expense can be read like: \n"You paid {amnt.get()} to {pay.get()} for {desc.get()} on {date.get_date()} via {MoP.get()}"'
...
dataentery_frame_bg = 'blue'
buttons_frame_bg = 'navyblue'
hlb_btn_bg = 'white'
...
Label(data_entry_frame, text='Pay\t             :', font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=230)
Entry(data_entry_frame, font=entry_font, width=31, text=pay).place(x=10, y=260)
...
# Treeview Frame
table = ttk.Treeview(tree_frame, selectmode=BROWSE, columns=('ID', 'Date', 'Pay', 'Description', 'Amount', 'Mode of Payment'))
...
table.heading('Pay', text='Pay', anchor=CENTER)
...
table.column('#3', width=150, stretch=NO)  # Pay column
